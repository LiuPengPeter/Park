...
ACMultiR1::computeQpResidual()
{
  ...
  for (unsigned int j = 0; j < _etas_num; ++j)
  {
    if (j==_index_var)
    continue;
    sum += 2/_Np*_x[_index_var]*_x[j]*_M[_index_var*_etas_num+j]*_E[_index_var*_etas_num+j]*_E[_index_var*_etas_num+j]/2*(_grad_u[_qp]-(*_grad_etas[j])[_qp])*_grad_test[_i][_qp];
  }
  return sum;
}
...
ACMultiR1::computeQpJacobian()
{
  ...
  for (unsigned int j = 0; j < _etas_num; ++j)
  {
    if (_index_var==j)
    continue;
    sum += 2/_Np*_x[_index_var]*_x[j]*_M[_index_var*_etas_num+j]*_E[_index_var*_etas_num+j]*_E[_index_var*_etas_num+j]/2*_grad_phi[_j][_qp]*_grad_test[_i][_qp];
  }
return -sum;
}
...
if (cvar >= 0)
  return 2/_Np*_x[_index_var]*_x[cvar]*_M[_index_var*_etas_num+cvar]*_E[_index_var*_etas_num+cvar]*_E[_index_var*_etas_num+cvar]/2*(-_grad_phi[_j][_qp]*_grad_test[_i][_qp]);
